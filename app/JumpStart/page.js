...
import cardData from './DataJumpStart';
...
export default function HomePage() {
  const addressee = 'JumpStart'; // CHANGE NAME OF ADDRESSEE
  ...
  const [contactVisible, setContactVisible] = useState(false); // To trigger contact fade-in
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        // Show contact after cards have faded in
        setTimeout(() => {
          setContactVisible(true);
        }, cardData.length * 400 + 1500);
